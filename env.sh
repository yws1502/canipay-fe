...
# Extract NEXT_PUBLIC_ environment variables and build the sed expression
...
if [ ! -s "$sed_script" ]; then
    echo "No NEXT_PUBLIC_ environment variables found. Exiting with error."
    exit 1
fi

# sed All files
# find /app/.next -type f -exec sed -i -f "$sed_script" '{}' +

# sed JS and CSS only
find /app/.next -type f \( -name '*.js' -o -name '*.css' \) -exec sed -i -f "$sed_script" '{}' +